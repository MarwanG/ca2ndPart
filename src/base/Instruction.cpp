...
bool Instruction::reads_in(int dst){
   
   if(this->get_nbOp()>2){
      //meme si on a 3 opérandes, faut vérifier que opreg!=null, car on peut avoir un oper immediat
      OPRegister *opr=this->get_reg_src1();
      if(opr!=NULL && opr->get_reg()==dst)
	 return true;

      opr=this->get_reg_src2();
      if(opr!=NULL && opr->get_reg()==dst)
	 return true;
   }
   else if(this->get_nbOp()==2 
	   && this->get_reg_src1()->get_reg()==dst){
      return true;
   }
...
bool Instruction::writes_in(int dst){
   OPRegister *rdst=this->get_reg_dst();
   if(rdst==NULL)
      return false;
   
   return rdst->get_reg()==dst;
}

bool Instruction::is_writed_between(int dst, Instruction *i1, Instruction *i2exclu){
   Instruction *itmp=i1->get_next();
   
   while(itmp!=NULL && itmp!=i2exclu){
      
      if(itmp->writes_in(dst)){
	 return true;
      }
